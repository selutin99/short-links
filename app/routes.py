...
def create_short_url():
    validate_request(request.json, create_link_request_schema, status.HTTP_400_BAD_REQUEST, valid_required=True)
...
def get_original_url(short_postfix):
    validate_request(request.path, preformat_request({"short_postfix": short_postfix}, get_link_request_schema),
                     status.HTTP_400_BAD_REQUEST, valid_required=False)
